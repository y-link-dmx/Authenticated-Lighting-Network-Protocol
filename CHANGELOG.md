...
- Move `alpine-protocol-rs` under `protocol/rust/` and keep the crate focused on wire helpers, crypto primitives, and stream profiles. `AlpineClient` now lives entirely in `sdk/rust/alpine-protocol-sdk`.
 - Introduce `protocol-publish.yml` and `sdk-publish.yml`, version the protocol artifacts for `v2.0.10`, and let every SDK release follow its own semantic version set (the Rust SDK is `0.1.0` for this cycle).
